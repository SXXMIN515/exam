...
CREATE TABLE board_tbl (
  `no` INT NOT NULL AUTO_INCREMENT, -- 키.
  `title` VARCHAR(1000) NOT NULL,
  `writer` VARCHAR(1000) NOT NULL,
  `content` VARCHAR(1000),
  `created_dt` DATETIME NOT NULL, -- 작성일자.
  `updated_dt` DATETIME DEFAULT CURRENT_TIMESTAMP, -- 수정일자.
  PRIMARY KEY (`no`)
  );
  
  -- 문제 1 2) 
  INSERT INTO `board_tbl` (`no`, `title`, `writer`, `content`, `created_dt`, `updated_dt`)
  VALUES 
...
CREATE TABLE comment_tbl (
  ...
  `bno` INT NOT NULL,
  PRIMARY KEY (`no`),
  FOREIGN KEY (`bno`) REFERENCES board_tbl (`no`)
  );
  
  -- 문제 1 4)
  INSERT INTO `comment_tbl` (`no`, `writer`, `content`, `created_dt`, `updated_dt`, `bno`)
  VALUES 
